...
MAIN_CMD = os.path.normpath(os.path.join(MAIN_CMD, "../dependencies/mkhdri.exe"))


def all_same(items):
    """Takes iterable and checks if each item is the same.

    (useful for comparing file extensions, for example)
...
def parse_cmd(string_h):
    """Parse a command-line command str so hdr_batch_process can use it.
...
def hdr_batch_process(inputs, seqInt, *args, **kwargs):
    """The hdr batch process utility's main function.
